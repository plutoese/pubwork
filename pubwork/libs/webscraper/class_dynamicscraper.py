...
class DynamicScraper:
    ...
    def surf(self,proxy=True,proxy_type='http',timeout=0,type=0,ready_check=None):
        """ 连接上网
        ...
        while(not successful):
            print('Starting Surf...................')
            if proxy:
                mproxy = self.pmanager.random_http_proxy if proxy=='http' else self.pmanager.random_https_proxy
                self.browser = HeadlessBrowser(proxy=mproxy,proxy_type=proxy_type,timeout=timeout,type=type)
            else:
                self.browser = HeadlessBrowser(proxy=None,proxy_type=proxy_type,timeout=timeout,type=type)
...
if __name__ == '__main__':
    ...
    dscraper.surf(proxy=True,proxy_type='https',timeout=5,type=0,
