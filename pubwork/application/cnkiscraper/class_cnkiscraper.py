...
from libs.webscraper.class_headlessbrowser import HeadlessBrowser
...
class CnkiScraper:
    ...
    def set_query(self,query_str=None):
        ...
        :return: 无返回值
        """
        self.browser.interact_one_time(location=r'#\31 _4 > a:nth-child(1)',click=True)
        time.sleep(2)
...
class CnkiScraperInterface:
    ...
    def query(self, querystr=None, period=None, subject=None, sortedby=None, limit=None):
        tida = 0
        sign = True
        while sign:
            try:
    ...
    def check_connection(self, max_connection_number=10):
        ...
        if self.proxy is not None:
            while not is_proxy_successful:
                ...
                self.cnki_obj = CnkiScraper(proxy=proxy,type=0)
            ...
            self.cnki_obj = CnkiScraper(type=0)
            if self.cnki_obj.is_connected():
                return True
            else:
                return False

    def insert_to_db(self, literatures=None, database='papers', collection='cnki',condition=None):
        ...
        for record in literatures:
            ...
            if condition is not None:
                if not condition[1] == record.get(condition[0]):
                    print('Journal not matched!->',condition[1],record.get(condition[0]))
                    continue

            result = conn.collection.find_one({'title':record.get('title'),
            ...
            if result is None:
                ...
                print('Update...!')
                conn.collection.update_one({'title':record.get('title'),
                                               'journal':record.get('journal'),
                                               'year':record.get('year'),
                                               'issue':record.get('issue'),
                                               'pages':record.get('pages')},
...
if __name__ == '__main__':
    ...
    journal_name = '世界经济文汇'
    for y in ['2010','2011','2012','2013','2014','2015','2016'][5:]:

        cnki_interface = CnkiScraperInterface(proxy=pmanager.top_50_proxies)
        ...
        cnki_interface.insert_to_db(literatures=literatures,collection='cliterature',
                                    condition=('journal',journal_name))
